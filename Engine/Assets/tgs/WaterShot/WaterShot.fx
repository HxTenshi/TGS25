...
float3 tri3(float3 p){ return float3(tri(p.z + tri(p.y)), tri(p.z + tri(p.x)), tri(p.y + tri(p.x))); }
...
PS_INPUT VS( VS_INPUT input )
{
	...
	float3 noisex = float3(input.Pos.zy*0.001, 1.2345);
	float3 noisey = float3(input.Pos.zx*0.001, 6.7891);
	noisex.x += Time.y*0.25;
	noisey.x += Time.y*0.25;
	float nxf = Noise3d(noisex);
	float nyf = Noise3d(noisey);
	
	input.Pos.x += ( nxf*2 -0.5)*20;
	input.Pos.y += ( nyf*2 -0.5)*20;
...
PS_OUTPUT_1 PS(PS_INPUT input)
{
	...
	Out.Diffuse = DifColor * MDiffuse;
	Out.Diffuse.a = DifColor.a * f;
